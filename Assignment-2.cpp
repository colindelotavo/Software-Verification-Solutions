...
void ICFGTraversal::dfs(const ICFGEdge *src, const ICFGNode *dst) {
    visited.insert({src, callstack});
    path.push_back(src);
    if (path.back()->ICFGEdge::getDstNode()->getId() == dst->getId()) {
        printICFGPath();
    }
    for (const ICFGEdge *edge : src->getDstNode()->getOutEdges()) {
        if (visited.find({src, callstack}) != visited.end()) {
            if (const IntraCFGEdge *intraEdge = dyn_cast<IntraCFGEdge>(edge)) {
                if (handleIntra(intraEdge)) {
                    dfs(intraEdge, dst);
                }
            } else if (const CallCFGEdge *callEdge = dyn_cast<CallCFGEdge>(edge)) {
                if (handleCall(callEdge)) {
                    callstack.push_back(callEdge->getCallSite());
                    dfs(callEdge, dst);
                    callstack.pop_back();
                }
            } else if (const RetCFGEdge *retEdge = dyn_cast<RetCFGEdge>(edge)) {
                if (handleRet(retEdge)) {
                    if (!callstack.empty()) {
                        callstack.pop_back();
                        dfs(retEdge, dst);
...
void ICFGTraversal::printICFGPath() {
    ...
    for (auto e : path) {
        pathString += to_string(e->ICFGEdge::getDstNode()->getId());
        pathString += "->";
    }
    pathString += "END";
